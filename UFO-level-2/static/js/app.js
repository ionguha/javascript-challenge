...
function filteredDataTable()
{
  ...
  if(formDateField.value.length != 0) {
    ...
    var subCondition = ["datetime",formDate];
  ...
  if(formCityField.value.length != 0) {
    ...
    var formCity = d3.select("#city").property("value").toLowerCase();
    // Create a list item with index as the first element
    var subCondition = ["city",formCity];
    // Append to the condition list
    condition.push(subCondition);
  }
  ...
  if(formStateField.value.length != 0) {
    ...
    var formState = d3.select("#state").property("value").toLowerCase();
    // Create a list item with index as the first element
    var subCondition = ["state",formState];
    // Append to the condition list
    condition.push(subCondition);
  }
  ...
  if(formCountryField.value.length != 0) {
    ...
    var formCountry = d3.select("#country").property("value").toLowerCase();
    // Create a list item with index as the first element
    var subCondition = ["country",formCountry];
    // Append to the condition list
    condition.push(subCondition);
  }
  ...
  if(formShapeField.value.length != 0) {
    ...
    var formShape = d3.select("#shape").property("value").toLowerCase();
    // Create a list item with index as the first element
    var subCondition = ["shape",formShape];
    // Append to the condition list
    condition.push(subCondition);
  }
 
  var conditionString = condition.map(item => "data." +item[0] + "===" +"'" +item[1] +"'" ).join(" && ");
  console.log(conditionString);
  // Filter data using map & arrow function
  var filteredDate = tableData.filter(data => eval(conditionString));
